    ...
    return <div>
        <div onClick={() => setIsOpen(true)} className="cursor-pointer">
            <div className="aspect-square rounded-2xl overflow-hidden relative group">
                {photo && <Image width={1920} height={1920} src={photo} alt="Image de galerie" className="image-cover group-hover:scale-[1.3] transition-all duration-700 cursor-pointer" />}
            ...
            <Dialog onClose={() => setIsOpen(false)} className="fixed inset-0 z-50 flex items-center">
                ...
                <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                    <Dialog.Panel className="container relative max-h-full">
                        {photo && <Image width={1920} height={1920} src={photo} alt="Image de galerie" className='object-contain' />}
                    </Dialog.Panel>
                </Transition.Child>
            </Dialog>
        </Transition>
